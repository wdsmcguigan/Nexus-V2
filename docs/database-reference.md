...
Canonical reference for the **31 tables + 2 FTS5 virtual tables** in the local SQLite vault, generated by reading `src-tauri/src/db/schema.rs` directly.
...
Last verified against `src-tauri/src/db/schema.rs`: 2026-06-19.
...
### Links graph (substrate)

| Table | Purpose |
|---|---|
| `links` | Typed edges between entities (substrate Pillar 3). Columns: `id`, `vault_id`, `src_type`, `src_id`, `link_type`, `dst_type`, `dst_id`, `meta_json` (nullable), `created_at`. Either endpoint may be a core entity (e.g. `nexus/email.message`) or a module entity (e.g. `org.nexus.tasks/task`). Written/removed via the `CREATE_LINK` / `DELETE_LINK` mutations; hydrated by `load_links`. See `docs/substrate-design.md` §6. |
...
| `idx_links_src` | `links(vault_id, src_type, src_id)` | Outgoing-edge lookup (substrate) |
| `idx_links_dst` | `links(vault_id, dst_type, dst_id)` | Incoming-edge lookup (substrate) |
